...
"""Busca los Usuarios con ID - Document y los Borra"""
def delete_client():
    ...
    try:
        ...
        if command == 1:
            ...
            conexion.commit()
            cursor.execute("DELETE FROM users WHERE ID = {} ".format(client_id))
            conexion.commit()
            cursor.close()
            os.system("cls")
            _print_welcome()
        ...
        if command == 2:
            ...
            client = cursor.fetchone()
            conexion.commit()
            cursor.execute("DELETE FROM users WHERE Document = {} ".format(client_document))
            conexion.commit()
            cursor.close()
            os.system("cls")
            _print_welcome()
